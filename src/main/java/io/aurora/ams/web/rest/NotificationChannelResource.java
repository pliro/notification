...
public class NotificationChannelResource {
    ...
    @RequestMapping(value = "/_search/notification-channels",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public List<NotificationChannel> searchNotificationChannels(@RequestParam String query) {
        log.debug("REST request to search NotificationChannels for query {}", query);
        return StreamSupport
            .stream(notificationChannelSearchRepository.search(queryStringQuery(query)).spliterator(), false)
