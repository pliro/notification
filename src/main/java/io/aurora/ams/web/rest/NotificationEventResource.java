...
public class NotificationEventResource {
    ...
    @RequestMapping(value = "/notification-events",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<NotificationEvent>> getAllNotificationEvents(Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to get a page of NotificationEvents");
        Page<NotificationEvent> page = notificationEventRepository.findAll(pageable); 
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/notification-events");
    ...
    @RequestMapping(value = "/_search/notification-events",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<NotificationEvent>> searchNotificationEvents(@RequestParam String query, Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to search for a page of NotificationEvents for query {}", query);
        Page<NotificationEvent> page = notificationEventSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/notification-events");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
